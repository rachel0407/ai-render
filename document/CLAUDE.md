...
| AI | Google `genai` SDK，模型 `gemini-2.5-flash-image` |
...
- 例外：`frontend/` 整個資料夾用 `:ro` mount 進 container（裡面有 `admin.html` / `customize.html` / `setup.html`），改完直接生效（要清瀏覽器快取）。

### 4. Setup Wizard 跟 `.env` 的優先級
- Settings 載入順序：**環境變數 / `.env`** > **`storage/config.json`**（wizard 寫入）> **default**
- 所以 `.env` 配好的 `GEMINI_API_KEY` 會蓋過 wizard 寫的；想用 wizard 重設，要先把 `.env` 那行清掉/註解掉
- 想完整重置：刪 `storage/config.json` + 清 `.env` 對應行，下次 `docker compose up -d` 或 `restart` 會回到 wizard
- wizard 用 in-memory `setattr(settings, ...)` 即時生效，**不用重啟 container** 就能登入後台
...
```
app/
├── main.py                  # FastAPI app + 路由 / /admin /setup
│                            # 沒 configured 時 / 跟 /admin 會 303 redirect 到 /setup
├── config.py                # Pydantic Settings；env > .env > storage/config.json > default
├── api/
│   ├── routes.py            # Setup: /setup/status, /setup
│   │                        # 公開: /render, /render-jobs, /remove-background, /sources
│   │                        # Admin: /admin/login, /admin/me, /admin/history,
│   │                        #        /admin/folders（CRUD）, /admin/folders/{f}/images（CRUD）
│   └── deps.py              # verify_admin_token（公開 endpoints 不需要 API key）
├── schemas/render.py        # 所有 Pydantic 模型（含 SetupStatusResponse / SetupRequest）
└── services/
    ...
    ├── setup_service.py     # Wizard 寫 storage/config.json + setattr settings 即時生效
    └── render/
        ├── dispatcher.py    # render provider 抽象層（目前只有 gemini）
        └── gemini.py        # Gemini 實作（render / render_composite）

frontend/                    # 整個資料夾 mount :ro 進 container
├── setup.html              #   首次啟動的設定 Wizard
├── customize.html          #   User 端
└── admin.html              #   後台前端
docker-compose.yml           # api service + Traefik labels
.env                         # 可選 secret（**不進 git**；沒設就走 wizard）
storage/config.json          # Wizard 寫入的設定（**不進 git**）
```
